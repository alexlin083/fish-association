...
function TNBoardSingle(props) {
  ...
  useEffect(() => {
    axios
      .get(`http://18.140.90.171:3000/api/travelNotes/boardview/${noteId}`)
      .then((serverResponse) => {
          ...
          <>
            <div className="d-flex my-3 align-items-center">
              <div className="TN-board-content d-flex justify-content-between align-items-end">
                <div>
                  <p style={{ fontSize: "20px" }} className="mb-1">
                    {boardData.board_usename}
                  </p>
                  <p style={{ fontSize: "16px" }}>{boardData.board_content}</p>
                </div>
                <p
                  style={{ fontSize: "12px", opacity: ".6", textAlign: "end" }}
                >
                  {moment(boardData.board_update_time).format(
                    "YYYY-MM-DD HH:mm:ss"
                  )}
                </p>
              </div>
